...
impl Screen {
    ...
    /// Reconstruct both screen buffers in a fresh viewer, without scrollback.
    /// The reset applies only to that viewer, never to the host terminal.
    #[must_use]
    pub fn snapshot_formatted(&self) -> Vec<u8> {
        let mut output = b"\x1bc".to_vec();
        if self.alternate_screen() {
            let mut primary = self.clone();
            primary.clear_mode(MODE_ALTERNATE_SCREEN);
            primary.attrs = self.saved_attrs;
            primary.set_scrollback(0);
            output.extend(primary.state_formatted());
            primary.grid().write_snapshot_scroll_region(&mut output);
            output.extend_from_slice(b"\x1b[?1049h");
        }
        let mut live = self.clone();
        live.set_scrollback(0);
        output.extend(live.state_formatted());
        live.grid().write_snapshot_scroll_region(&mut output);
        output
    }
...
impl Screen {
    ...
    pub(crate) fn ed(
        ...
        match mode {
            ...
            2 => self.grid_mut().erase_all_preserving_scrollback(attrs),
