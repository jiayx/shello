...
impl Grid {
    ...
    pub fn write_snapshot_scroll_region(&self, contents: &mut Vec<u8>) {
        if self.scroll_top == 0 && self.scroll_bottom == self.size.rows - 1 && !self.origin_mode {
            return;
        }
        contents.extend_from_slice(format!("\x1b[{};{}r\x1b[?6{}",
            self.scroll_top + 1, self.scroll_bottom + 1,
            if self.origin_mode { "h" } else { "l" }).as_bytes());
        // Cursor addressing is relative to the scroll region in origin mode.
        let row = if self.origin_mode { self.pos.row.saturating_sub(self.scroll_top) } else { self.pos.row };
        contents.extend_from_slice(format!("\x1b[{};{}H", row + 1, self.pos.col.min(self.size.cols - 1) + 1).as_bytes());
    }
    ...
    pub fn erase_all_preserving_scrollback(&mut self, attrs: crate::attrs::Attrs) {
        if self.scrollback_len > 0 && !self.scroll_region_active() {
            let count = self.rows.iter().rposition(|row| {
                (0..self.size.cols).any(|col| row.get(col).is_some_and(crate::Cell::has_contents))
            }).map_or(0, |index| index + 1);
            self.scrollback.extend(self.rows.iter().take(count).cloned());
            while self.scrollback.len() > self.scrollback_len { self.scrollback.pop_front(); }
            if self.scrollback_offset > 0 {
                self.scrollback_offset = self.scrollback.len().min(self.scrollback_offset + count);
            }
        }
        self.erase_all(attrs);
    }
